...
- **Cycle de vie des plans** :
  ...
  - `GET /api/plans/plans/for-sites/?site_ids=1,2` - **Plans validés/archivés associés à un/des site(s)**, groupés par site et triés par rang (statuts `VALIDATED_STATUSES` = valide, modifie, archive ; brouillons exclus). Scopé aux permissions de l'utilisateur (`get_queryset`). Sert à la **création standard d'un PG** (page « Créer un plan ») pour : (1) alerter si un PG du même rang existe déjà sur le site, (2) proposer le rattachement au plan du rang précédent, (3) afficher le détail des PG déjà rattachés.
  - **Rattachement automatique du rang à la création standard (fil entre rangs)** : `POST /api/plans/plans/` accepte un `plan_parent_id` optionnel. `perform_create` ne le pose **qu'après validation serveur** via `PlanGestionViewSet._is_valid_rang_parent(parent, plan)` — parent dans `VALIDATED_STATUSES`, **rang strictement inférieur**, et **au moins un site en commun**. Sinon le lien est ignoré (`plan_parent` reste NULL). Permet de conserver la chaîne de versions quand un nouveau rang est créé depuis la page d'accueil (et pas seulement via `create-next-rang`). Le front (page de création) propose une **case « Rattacher ce plan à la suite de … »** cochée par défaut ; le parent suggéré est le plan validé de rang le plus élevé strictement inférieur au rang saisi (puis version max) parmi les sites sélectionnés. Décocher la case → plan indépendant.
...
- À la sélection d'un/des site(s), un `effect()` appelle `adminService.getPlansForSites(siteIds)` (endpoint `for-sites`) et stocke les PG validés/archivés dans le signal `existingPlansBySite`.
- **Détail des PG existants** : bloc dépliable listant, par site et par rang, les plans validés/archivés du/des site(s) sélectionné(s).
...
- **Rattachement au rang précédent (chaîne de versions)** : computed `suggestedParent()` (rang le plus élevé < rang saisi, puis version max, parmi les sites). Case `app-checkbox` « Rattacher ce plan à la suite de … » (signal `linkToParentEnabled`, cochée par défaut) ; si cochée, `plan_parent_id` est ajouté au payload `createPlan`. Le backend revalide le parent (cf. `_is_valid_rang_parent`).
